...
-- Recria a função com melhor tratamento de erros
...
BEGIN
    ...
    IF EXISTS (
        SELECT 1 FROM auth.users WHERE email = create_auditor.email
    ) THEN
    ...
    IF EXISTS (
        SELECT 1 FROM profiles WHERE email = create_auditor.email
    ) THEN
